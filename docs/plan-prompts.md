...
## 0. État au 7 août (vagues 1–2 fermées, vague 3 aux ¾, Phase 5 design en cours)

> **Mise à jour 2026-08-17** : depuis ce point — vague 3 FERMÉE (P-16, 11/08),
> vague 4 CMS livrée et poussée (Contact/Carrières/pages système éditables,
> 12/08, `f757c35`), design system v3 « export2 » appliqué (14/08, `47c1a19`),
> puis semaine « fermeture maximale » (17/08, `c69a8e5`) : **P-08 ✅, P-11 ✅
> côté code** (activation = clés, operations.md §7ter), portage formulaire
> Contact, breadcrumbs partout, schémas Service/LocalBusiness, filtre Type
> recherche, maillage `topics` (F3.2), P-20 avancé (+6 e2e). Le pilotage des
> fermetures se fait désormais dans **`docs/ado-alignement.md` (v2, IDs
> réels)** ; le journal §9 détaille chaque session. Le reste de ce §0 est
> conservé comme photo du 7 août.

**Fermés et poussés depuis le point du 28/07** : P-04 (header de landing),
P-06 (Pagefind), P-10 (bandeau Loi 25), P-12 (BreadcrumbList), P-14 (RSS),
P-15 (style-guide) — les 🟡 du tableau sont passés ✅ (code commité sur
`spike/cloudcannon`, gate vert à chaque lot). **Gate CloudCannon CLOSE
formellement le 2026-08-07** (grille remplie dans `spike-cloudcannon.md` ;
seul le critère 8 — vidéo — reste, replié dans P-21). **Phase 5 design** :
palette de 30 sections entièrement re-skinnée (2026-08-06), pages Accueil/
Contact/Carrières/Expertise mère/Produit enfant fidèles aux maquettes,
catalogue Solutions livré (collection + route + filtres) et **déclaré au CMS
...
| P-04 | Header de landing par page (complet/allégé/personnalisé) | ~~Sonnet 5~~ **Fable 5** | P-01; en V2 après P-07 | 3,5 h | 2 | ✅ | Fermé — commité/poussé depuis (statut mis à jour 07/08). Exécuté 30/07 (session directe). Implémentation par COMPOSANTS DÉDIÉS `CampaignHeader`/`CampaignFooter` + slots nommés remplis PAR LA ROUTE campagnes (pas d'import dans BaseLayout — son graphe de modules alimente toutes les pages, le CSS aurait fui partout ; Header/Footer historiques INTOUCHÉS). Frontmatter landing : bloc `header` (mode complet/allege/personnalise, links≤5, ctaLabel/ctaHref plats — cascade _inputs existants, 3 switches) + `footerMode` PLAT (collision de cascade CloudCannon avec le `mode` de l'en-tête sinon). Politique dans `src/lib/chrome/header-config.ts` (10 tests) : allégé = logo + CTA STRICT (repli bouton portail), recherche opt-in hors complet (amendement P-06). Piège Astro constaté : expression FAUSSE portant slot=… supprime le contenu par défaut → ternaire explicite Header/Footer. Démo : evaluation-securite fr/en en allégé. Gate vert (lint 0 err, 122 tests, type-check 0 err, build + STATIC_ONLY, e2e 7/7 dont 3 nouveaux header-variants.spec.ts) ; parité prouvée par double build isolé (sitemaps identiques, diff /fr/contact = P-10/P-14 seulement). Reste : vérif CloudCannon humaine (bloc « En-tête de la page » sur une campagne). |
...
| P-08 | Courriel de confirmation visiteur (2ᵉ envoi SMTP2GO) | ~~Opus 4.8~~ **Fable 5** | P-05 | 3,5 h | 3 | ✅ | Fermé 2026-08-17 (`c69a8e5`) — `src/lib/forms/confirmation.ts` (gabarit fixe FR/EN, non bloquant) + 4 tests. Activation réelle = clés SMTP2GO (operations.md §7ter). |
| P-09 | — fusionné dans P-05 (conditionnels = mêmes contrats) | — | — | — | — | — | |
| P-10 | Bandeau Loi 25 minimal (tokens) + mécanique de gating | ~~Opus 4.8~~ **Fable 5** | — (GO utilisateur 17/07) | 10,5 h | 3 | ✅ | Fermé — commité/poussé depuis (statut mis à jour 07/08). Exécuté 30/07 (session directe). `ConsentBanner.astro` (BaseLayout, toutes pages) : design minimal tokens, Accepter/Refuser, lien politique, chaînes ui.ts FR/EN. Contrat P-11 : localStorage `victrix-consent`, classe `consent-analytics` sur `<html>`, CustomEvent `victrix:consent`, gating générique `<script type="text/plain" data-consent="analytics">` (activation au consentement, y compris retours + navigations ClientRouter). Vérifié en navigateur : accepter → script gated exécuté ; refuser → inerte ; persistance OK. Reste : P-11 (pose des scripts) toujours bloqué sur OPS-CSP + clés. |
| P-11 | Pose GA4/GTM/Clarity + événements conversion + Site Search + objectifs | ~~Sonnet 5~~ **Fable 5** | P-10 + P-06 + OPS-CSP | 7 h | 3 | ✅ | Fermé côté code 2026-08-17 (`c69a8e5`) — gtag GELÉ sous consentement (BaseLayout), conversion `generate_lead` (/merci), Site Search (/recherche), **CSP appliquée**. Activation = `PUBLIC_GA4_ID` (§7ter). Clarity/heatmap : décision en attente (ado-alignement #1451). |
| P-12 | Fil d'Ariane + BreadcrumbList JSON-LD | ~~Sonnet 5~~ **Fable 5** | — | 3,5 h | 3 | ✅ | Fermé — commité/poussé depuis (statut mis à jour 07/08). Exécuté 30/07 — PÉRIMÈTRE AJUSTÉ : le fil d'Ariane VISIBLE existait déjà sur les articles (parité victrix.ca) ; ajouté = balisage `BreadcrumbList` JSON-LD (`src/lib/seo/breadcrumb.ts`, 2 tests) sur articles (Accueil›Ressources›titre) et services (Accueil›titre — pas de niveau « Services » balisé, IA d'URLs provisoire P-17/P-18). Fil visible sur les autres gabarits REPORTÉ au redesign (décision : ne pas préempter la Phase 5). |
...
| P-20 | Extension QA Playwright | ~~Sonnet 5~~ **Fable 5** | vagues 1–3 | 7 h | 4 | 🔵 | Avancé 2026-08-17 : +6 e2e (formulaires maquette, formId sur service, fil d'Ariane ×2, recherche, merci) + flake portail corrigé (timeout routes à la demande). Reste : Lighthouse/axe CI, cross-browser. |
...
| P-23 | Planification étendue : fenêtres de dates sur les SECTIONS (accueil, campagnes) — patron de la bannière (src/lib/schedule.ts + 2 champs par section, évaluation au build via le seam enrich ou le renderer) | Sonnet 5 | mécanique 30/07 (bannière+articles) | 7 h | backlog | ⬜ | Demande équipe 30/07 (« scheduling changes on homepage »); la bannière promo et les articles programmés sont FAITS — ceci étend aux sections composables. Prérequis OPS : secret REBUILD_HOOK_URL branché. |
| P-24 | Méga-menu UNIVERSEL : CHAQUE entrée du header ouvre le méga-menu (il reste OUVERT au survol continu d'un item à l'autre — pas de fermeture/réouverture), colonnes de liens + zone d'appel à l'action CONTEXTUELLE par section (remplir l'espace : CTA lié + liens complémentaires), et pied de méga FIXE identique partout (« Études de cas » + « Livres blancs » toujours visibles) | Fable 5 | P-01 (nav CMS) | 7 h | backlog | ⬜ | Idée user 2026-08-24 (session composants produit). Toucher : Header.astro (accroche méga par URL RÉSOLUE — garde-fous #3 revue P-07 à préserver), `nav_columns`/CMS étendus à toutes les entrées + nouveau bloc CTA par colonne, pied commun sourcé une seule fois, transitions hover sans fermeture inter-items, a11y clavier/aria et nav mobile intactes. |
...
|---|---|---|
| 2026-08-25 | Fable 5 — **BATCH COMPLET : les 21 pages services restantes restylées à la recette (42 fichiers fr+en, P-19/P-12 présentation + #1412/#1480)** — GO user (« every page has a nice presentation »). Exécution par 4 agents parallèles (Productivité enfants=style PRODUIT product-hero ; Cybersécurité/Infonuagique/TI gérés/Conseil+IA=service-hero conservé, corps recette) sur brief partagé (scratchpad brief-recette-produit.md : contrats zod exacts, pièges, autocontrôle). **Interruption limite de session mi-course** → reprise ciblée par SendMessage après scan déterministe (scan-batch.mjs : matrice parse+types) — 4 fichiers restants complétés. **Pré-vérificateur écrit (check-batch.mjs)** : enums/préfixes langue/HTML-dans-faq/double-héros/related-posts multiples — 0 problème sur 42. RÈGLE no-invention TENUE (spot-check : 21+/+300/+7500 et ISO 27001:2022 de la page cyber = copie migrée VERBATIM). Notes des agents : liens retirés des contextes texte-brut (consigné par page), 2 réparations EN d'artefacts de migration (servicenow phrase amputée, analyse-ia étape dupliquée — nouvelles formulations signalées), d365fs 9 sections et accompagnement 11 sections (assumé — matière dense), 5e carte realisations d'accompagnement passe sur 2e rangée (à regarder). Gate : lint 0 err, 128 tests, type-check 0, builds prod+STATIC_ONLY **162 pages** verts (vvbuild §3.1), dist : h1 uniques, form cyber intact (#formulaire), 0 fuite. Captures cyber+copilot envoyées. NON touchés : 3 placeholders (infrastructure/projets-en-ia/services-applicatifs — contenu réel à fournir), parents IA/Productivité et copilot-studio (déjà finis), Carrières (décision A/B toujours ouverte). | **Tout le catalogue services au design raffiné** 🟡 (commit user) ; rien en ligne avant commit+push |
| 2026-08-24 (2) | Fable 5 — **P-17 (cœur) : O Studio réel + Découvrir Victrix restylés à la recette produit** (GO user après validation de demo-produit). (1) **`services/{fr,en}/productivite/o-studio.json` RÉÉCRITS** (murs rich-text → 9 sections : product-hero [photo WP réelle bg-o-studio, CTA ancrés `#formulaire` — ancre STABLE rendue par form.astro], bento-metrics, offer-cards [puces réelles], strategic-value gouvernance, realisations [4 cas alignés catalogue /solutions], **faq (3 Q/R réelles — texte BRUT : le composant ne rend PAS de HTML dans answer) → FAQPage JSON-LD émis (#1486)**, related-posts [tags FR « Productivité » / EN « Productivity » — les topics sont PAR LANGUE], form, cta nuit). Slug EN `productivity-consulting/o-studio` conservé — **URL de l'AC #1481 (/services/o-studio/) NON tranchée** (IA d'URLs = P-17/P-18) ; 301 vieux domaine = OPS (DNS hors dépôt). (2) **Formulaire DÉDIÉ `forms/{fr,en}/o-studio.json`** (patron campagne-evaluation : select « Votre besoin » + champ conditionnel Autre, consentement Loi 25, cachés page/UTM ; toEmail "" = repli). (3) **`pages/{fr,en}/decouvrir.json` restylés** (hero→service-hero overlay blanc badge « Depuis 2003 » [eyebrow texte = bleu pâle ILLISIBLE sur héros clair → badge], rich-text culture→strategic-value avec photo, NOUVEAU bento-metrics « Victrix en un coup d'œil » [2003/3 bureaux/ISO + carte Happy At Work→/carrieres], valeurs benefits AVEC icônes, histoire +2024 COMITEM côté EN [parité], cta→nuit ; noindex CONSERVÉ). (4) **chiffres-officiels.md** : +4,5×/-40 % (chiffres MAQUETTE jamais validés), ISO/HappyAtWork repris sur /decouvrir, consentement Ø Studio à la passe juridique. Gate : lint 0 err, 128 tests, type-check 0, builds prod+STATIC_ONLY 162 pages (vvbuild §3.1), dist : h1 uniques, cartes ressources RÉELLES, FAQPage présent, EN slug OK. Captures envoyées au user. | O Studio + Découvrir 🟡 (commit user) ; reste #1481 : décision URL /services/o-studio/ + 301 domaine (OPS) ; batch suivant : enfants Productivité |
| 2026-08-24 | Fable 5 — **Recette « page produit » : 2 nouveaux composants Bookshop (maquette docs/produits.css « Studio de création », déposée par le user avec docs/carriere.css)**. (1) **product-hero** (héros catalogue CLAIR : badge-pastille à point bleu, H1 72/72 ls -3.6 à sous-chaîne accent bleue, lead 20/32, boutons PILULES [géométrie propre au style catalogue], photo inclinée 2° sur halo bleu flouté ; AJUSTEMENT a11y AA du badge — fond primary/20 + texte primary-container, la maquette plafonnait ≈4.3:1 ; unique <h1>, jamais cumulé avec service-hero). (2) **bento-metrics** (tête centrée + carte Bleu nuit [titre 30, texte blanc 70 % — idiome cta « nuit », puces-métriques « verre » {value,label} = _structures.stat_tiles, pictogramme FILIGRANE blanc 10 %] + carte claire [tuile icône bleue, lien à flèche] ; clés `watermark`/`icon` DISTINCTES à dessein — un même nom partagerait sa config _inputs, gotcha 2026-07-20). Chacun : .astro browser-safe 100 % Tailwind + .bookshop.yml FR + union zod +2 (content.config.ts, groupe « catalogue produit ») + preview.png capturé. (3) **Pages démo `services/{fr,en}/demo-produit.json`** (noindex) = LA recette complète des pages enfants : product-hero → bento-metrics → offer-cards → strategic-value → realisations → cta « nuit » (contenu maquette Ø Studio ; liens EN sur slug surchargé productivity-consulting). (4) **design:previews** : SOURCES +demo-produit (garde-fou couverture 32/32 vert, capturé sur le dev server ACTIF du user — script read-only, aucun build), les 30 preview.png pré-existants RESTAURÉS (git checkout — le churn de re-capture appartient au Lot 6). (5) **P-24 backlog** (méga-menu universel — idée user consignée). Gate : lint 0 err/4 warns pré-existants, **128 tests**, type-check 0 err/4 hints + builds prod & STATIC_ONLY **162 pages** verts (copie isolée vvbuild §3.1, dev server actif), dist demo-produit fr+en : 1 seul h1, zéro fuite. NON TOUCHÉ : couleurs Carrières (décision A/B en attente — voir conversation). | Recette produit 🟡 (commit user) ; prochain : validation visuelle /fr/services/demo-produit/ puis « Go O Studio » (P-17/#1481) |
| 2026-08-17 | Fable 5 — **Semaine « fermeture maximale » ADO : P-08 + P-11 + fil d'Ariane + schémas + filtres recherche + maillage** (plan approuvé user ; lots 1-5). (1) **OPS-CSP appliquée** (public/_headers) : Turnstile (script/frame/connect) + GA4 (googletagmanager, *.google-analytics) — l'édition planifiée depuis P-05/P-11 ; operations.md §7ter (guide de pose des clés), .env.example +PUBLIC_GA4_ID, formulaires.md §CSP/§10 à jour. (2) **P-11 code** : loader gtag GELÉ (type=text/plain data-consent, contrat ConsentBanner) dans BaseLayout (rien sans PUBLIC_GA4_ID ; re-config par navigation = page_view SPA), conversion `generate_lead` sur /merci, Site Search sur /recherche (?q= + saisie Pagefind débouncée, garde anti-doublon). (3) **Formulaires bout-en-bout** : section form « Évaluation posture sécurité » (formId campagne-evaluation) sur cybersecurite FR+EN ; **portage page Contact** sur /api/forms (2 modes, _formId=contact, noms dérivés de la définition via fieldName, 2 GARDE-FOUS build page↔définition [libellés, options des selects], pot de miel, bannière ?erreur, Turnstile) — définitions contact.json fr/en réécrites mirror de la page ; **P-08** : 2e envoi SMTP2GO au visiteur (gabarit fixe par langue src/lib/forms/confirmation.ts, échec non bloquant, +4 tests). (4) **Fil d'Ariane VISIBLE** (composant Breadcrumbs.astro [data-crumbs], routes services+pages, PARENT inclus sur les services enfants, même tableau que le JSON-LD ; sélecteur du comptage design:previews exclut [data-crumbs]). (5) **Schémas** : Service (pages services indexables), LocalBusiness ×3 bureaux (page Contact, depuis la collection contact). (6) **Filtre « Type » Pagefind** : prop contentType de BaseLayout stampée sur <main> (Service/Article/Page), UI auto. (7) **Maillage F3.2** : nouveau champ blog `topics` (distinct des tags-catégories — filtres/méga intacts), 54 articles thématisés (script rejouable scratchpad), matchers services+pages étendus (tags∪topics), 26 sections « Ressources liées » posées, input CloudCannon + guide-edition. (8) **E2e +6** (tests/e2e/formulaires-recherche.spec.ts : contact maquette+validation, formId sur service, fil d'Ariane parent, recherche, merci) — à exécuter APRÈS redémarrage du dev server. Gate : lint 0 err, **126 tests** (+4), type-check+builds vvbuild (2 passes — la 2e post-maillage). RESTE : audit AC ADO (az login user en attente), clés OPS (user), Lighthouse/axe (reporté), previews/photos/Inter (Lot 6, exige dev server frais). | Semaine 🟡 (commit user) ; fermetures ADO pass 2 après audit + clés |
| 2026-08-12 | Fable 5 — **Vague 4 CMS : éditabilité complète + alignement ADO**. (1) **Pages Contact + Carrières éditables au CMS** : collections `contact`/`carrieres` (src/data/<coll>/{fr,en}.json, patron « Textes du site » verrouillé), contenu migré 1:1 de `src/i18n/content/{contact,carrieres}.ts` (supprimés), gabarits branchés sur `getLocaleData()` (nouveau helper générique `src/i18n/locale-data.ts`, gotcha Bookshop appel-nu respecté) ; `testimonials.items`→`cards` (collision carte plate `_inputs`). (2) **Pages système éditables** : collection `pagesSysteme` (blocs ressources/recherche/merci) — index blogue, fil d'Ariane des articles ET flux RSS lisent la même source ; ui.ts réduit à l'interface (a11y, Pagefind, partage). (3) **Config CloudCannon** : 3 collections FR (groupe Contenu du site) + 4 structures scopées (bureaux, valeurs, atouts, témoignages — 2 selects d'icônes distincts via _inputs de structure) ; **fix médias** : input image du méga-menu doté de `paths` publics (`public/images/nav` — tombait sur le repli global), `uploads/` vestige spike supprimé. (4) **Docs** : `docs/ado-alignement.md` (mapping backlog ADO ↔ réel : 16-18 Fermer, 5 Retirer obsolètes WP, correspondances P-xx), `docs/contenu-a-fournir.md` (placeholders à remplir + pas-à-pas ouverture CloudCannon), guide-edition (+3 sections : Contact/Carrières, Pages système, Médias ; promesse « tout le contenu » rendue exacte). Gate : lint 0 err/5 warns pré-existants, **122 tests**, type-check + builds prod & STATIC_ONLY dans vvbuild (dev server actif). | Vague 4 🟡 (commit user) ; vérifs CloudCannon (3 nouvelles collections) au retour |
| 2026-07-30 (2) | Fable 5 (mode auto) — **Consolidation architecture + Phase 3 + planification marketing**. (1) **Expertises→Services** (confirmation user) : collection/route/contenus `expertises` supprimés, 301 vers `/services/intelligence-artificielle` (pré-i18n + localisées), menu « Services » + méga-menu re-lié (`/services/*`, IA via champ `service` — résolu de la collection, garde-fou build), footer + cartes accueil re-liés — 0 lien `/expertises/` résiduel dans le dist. (2) **Phase 3** : Sveltia retiré (public/admin, CSP /admin, robots) — OPS : décommissionner le worker OAuth. (3) **UX CMS** : `collection_groups` (Contenu du site / Marketing / Configuration). (4) **Planification** (demande équipe) : bannière promo à fenêtre [Diffuser à partir de, Retirer à partir de) (zod garde-fou date invalide, éditeur voit toujours la bannière, `src/lib/schedule.ts` + 10 tests), articles à date future = différés (politique brouillons), workflow `.github/workflows/rebuild-planifie.yml` (cron 06:17 UTC + manuel; **OPS : secret REBUILD_HOOK_URL**), docs (operations §7bis, guide-edition « Planifier » + « Créer une page Services »), P-23 backlog (sections). Gate : lint 0, **110 tests** (+10), type-check 0, builds prod (23 pages Pagefind) + STATIC_ONLY verts; vérifs dist : redirect expertise→service ✓, admin absent ✓, méga-menu IA résolu ✓, bannière rendue ✓, index 7+7 ✓. PIÈGES : `Set-Content -Encoding utf8` (PS 5.1) écrit un BOM → `JSON.parse` du loader Astro casse (réécrits sans BOM via .NET); `$home` = variable RÉSERVÉE PowerShell. | Bloc auto 🟡 (commit user); re-tests CC au retour |
| 2026-07-30 | Fable 5 — **HOTFIX éditeur visuel « Error rendering page! » sur les pages services** (rapporté par l'utilisateur sur intelligence-artificielle). Cause : le garde-fou related-posts du renderer partagé (`page.astro`, constat #2 de la revue P-07) se déclenchait AUSSI dans le re-rendu NAVIGATEUR de l'éditeur — où `enrich` est TOUJOURS absent, même sur un service légitime (l'entête du fichier le documentait déjà). Fix : garde conditionné à `isBuildRender = Boolean((import.meta.env ?? {}).SSR)` (accès défensif, patron form.astro — import.meta.env peut manquer dans le bundle Bookshop navigateur); l'éditeur retombe sur la maquette factice ÉDITABLE du composant (isLive), comme conçu. Preuves : gate vert (lint 0, 100 tests, type-check 0), builds prod + STATIC_ONLY verts (vvbuild), **build NÉGATIF re-prouvé** (related-posts sur une campagne → exit 1, message FR intact). Reste : re-test humain de l'éditeur visuel sur un service après push. | Hotfix 🟡 (commit user + re-test CC) |
