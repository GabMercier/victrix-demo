...
| P-10 | Bandeau Loi 25 minimal (tokens) + mécanique de gating | ~~Opus 4.8~~ **Fable 5** | — (GO utilisateur 17/07) | 10,5 h | 3 | 🟡 | Exécuté 30/07 (session directe, non commité). `ConsentBanner.astro` (BaseLayout, toutes pages) : design minimal tokens, Accepter/Refuser, lien politique, chaînes ui.ts FR/EN. Contrat P-11 : localStorage `victrix-consent`, classe `consent-analytics` sur `<html>`, CustomEvent `victrix:consent`, gating générique `<script type="text/plain" data-consent="analytics">` (activation au consentement, y compris retours + navigations ClientRouter). Vérifié en navigateur : accepter → script gated exécuté ; refuser → inerte ; persistance OK. Reste : commit utilisateur ; P-11 (pose des scripts) toujours bloqué sur OPS-CSP + clés. |
| P-11 | Pose GA4/GTM/Clarity + événements conversion + Site Search + objectifs | Sonnet 5 | P-10 + P-06 + OPS-CSP | 7 h | 3 | ⬜ | §7 |
| P-12 | Fil d'Ariane + BreadcrumbList JSON-LD | ~~Sonnet 5~~ **Fable 5** | — | 3,5 h | 3 | 🟡 | Exécuté 30/07 (session directe, non commité) — PÉRIMÈTRE AJUSTÉ : le fil d'Ariane VISIBLE existait déjà sur les articles (parité victrix.ca) ; ajouté = balisage `BreadcrumbList` JSON-LD (`src/lib/seo/breadcrumb.ts`, 2 tests) sur articles (Accueil›Ressources›titre) et services (Accueil›titre — pas de niveau « Services » balisé, IA d'URLs provisoire P-17/P-18). Fil visible sur les autres gabarits REPORTÉ au redesign (décision : ne pas préempter la Phase 5). |
| P-13 | Footer 3 bureaux + carte façade | Sonnet 5 | P-01 (zone ui.ts) | 3,5 h | 3 | ⬜ | §7 |
| P-14 | RSS + partage social sans témoins | ~~Sonnet 5~~ **Fable 5** | — | 3,5 h | 3 | 🟡 | Exécuté 30/07 (session directe, non commité) — PÉRIMÈTRE AJUSTÉ : les boutons de partage sans témoins existaient déjà sur les articles (LinkedIn/X/Facebook/copier). Ajouté = flux RSS par langue `/{fr,en}/rss.xml` (`@astrojs/rss`, NOUVELLE dépendance) : articles publiés, URLs canoniques absolues, `<language>` fr-ca/en-ca + `<link rel=alternate type=application/rss+xml>` dans BaseLayout (découverte auto). |
| P-15 | Page style-guide (tokens + vitrine palette, noindex) | ~~Sonnet 5~~ **Fable 5** | P-02 | 3,5 h | 3 | 🟡 | Exécuté 30/07 (session directe, non commité). `/fr/style-guide/` (noindex, hors sitemap, monolingue — régime design-lab) : tokens (couleurs/typo/espaces/rayons/ombres, rendus en var(--…) live depuis tokens.css) + vitrine des 23 sections SANS duplication de données (recompose landing demo-sections + benefits/cta d'evaluation-securite + accueil + service demo-sections — mêmes sources que le script de captures ; enrich local : formulaires résolus + cartes factices related-posts). Page de contrôle visuel pour la « re-peau » Phase 5. |
