...
| `npx @bookshop/generate` local dit « Could not find any output sites » | Normal hors de CloudCannon — cherche `_cloudcannon/info.json`, généré seulement par leur environnement de build | Rien à corriger ; se fier au journal de build CloudCannon (§7). Pour REJOUER l'étape en local (vérifié le 2026-09-25) : après `STATIC_ONLY=1 EDITOR_PREVIEW=1 npm run build`, écrire un stub `dist/_cloudcannon/info.json` contenant `{"time":"2026-01-01T00:00:00Z","generator":{"name":"astro"},"_structures":{}}` puis `npx @bookshop/generate` — attendu « Added live editing to N pages » (128 le 25/09, 213 pages), `dist/_cloudcannon/bookshop-live.js` (~2 Mo), 42 structures dans `info.json` |
| **Édition visuelle morte sur un site CloudCannon** (aucun crayon au clic, sections non déplaçables) alors que le build local pose ses marqueurs | Le site a été construit SANS `EDITOR_PREVIEW` (variable absente, mal nommée, posée sur l'autre site, ou pas de rebuild depuis qu'elle a été posée — CloudCannon ne rebâtit pas tout seul après un changement de Site Settings) ; le journal de build dit alors « No live editing connected ». Diagnostic du 2026-09-25 (lot L-edition-visuelle) : `lawful-hare` 0 marqueur / 137 URL au sitemap / brouillon en 301, `vocal-wren` 5 marqueurs / 150 URL / brouillon en 200 — le code livrait Bookshop, seule la variable manquait | Trois sondes SANS l'UI, sur `https://<site>.cloudvent.net` : (1) `curl.exe -s …/fr/services/cybersecurite/ \| grep -o bookshop-live \| wc -l` → 0 = pas d'aperçu, ≥ 2 = Bookshop attaché, et `grep -c cloudcannon:load` = 1 si le postbuild a injecté le connecteur ; (2) `curl.exe -s …/sitemap-0.xml \| grep -o "<loc>" \| wc -l` → nombre de la PRODUCTION (sans les brouillons) = variable absente ; (3) l'URL d'un article `draft: true` → 301/404 = production, 200 = aperçu. Puis dans l'UI : Site Settings → Build → Environment Variables (les DEUX variables, valeur `1`, sans espace) → **Rebuild** (Builds → « Build » à la main) → journal : « Added live editing to N pages » |
