...
| L-menu-mobile | **Le méga-menu n'avait qu'un niveau sur téléphone — corrigé.** Le tiroir (`src/components/Header.astro` § « Mobile drawer ») ne rendait que les 6 entrées + Recherche. Désormais chaque entrée à `megaKey` est un accordéon natif `<details>`/`<summary>` sans script : le lien de la page mère en tête, puis les liens de SA colonne du méga-menu (3 accordéons de 4-5 liens, pas 3 × 14 ; repli = tout le panneau avec ses têtes de colonne si aucune colonne ne correspond) ; Ressources = lien vers le centre + catégories du blogue. Chevron CSS, jetons du thème, `aria-current` sur la page mère (rubrique) et sur la page courante (exact), rubrique dépliée d'office sur ses pages, fermeture au clic inchangée (script existant), carte vedette et bandeau non repris. `tests/e2e/menu-mobile.spec.ts` (6 tests : FR/EN Services → Cybersécurité, liens simples, colonne Expertises, Ressources, clavier + focus visible, `aria-current`) + tiroir ouvert FR/EN comme gabarit axe (0 violation). Vérifié : « Intelligence artificielle » n'a PAS d'`href` parce qu'il est en mode `service` (`intelligence-artificielle`) — résolu par la collection Services en `/fr/services/intelligence-artificielle/` et `/en/services/artificial-intelligence/` (slug EN de la page), validé au build ; rien à corriger. Aucun champ CMS nouveau | 0,5 j | — | | 2026-09-24 |
| L-cta-articles | **5e patron du corps d'article : le bandeau d'appel à l'action** (`<div class="article-cta">` — surtitre facultatif, titre, texte Markdown, un ou deux boutons `btn`/`btn-outline` sur une seule ligne), comme le composant `cta` des pages. Deux peaux : ivoire (défaut, filet contour) et `article-cta--marine` (bleu nuit, textes inversés, bouton plein blanc/bleu + contour blanc). 100 % CSS sous `.prose` dans `global.css`, zéro script (rangée de boutons = `p:has(> a.btn)`), contrastes AA vérifiés (blanc/nuit 19:1, surtitre 11:1 et 6,2:1, bouton 6,5:1 ; axe 0 sur la démo). Démo `/fr/style-guide/forme-articles/` (2 bandeaux), `docs/plan-forme-articles.md` § 5, guide § « Mettre en forme un article » modèle 5, `forme-articles.spec.ts` +2 (rangée sur écran large, peau marine, repli à 390 px). Aucun article touché (Julie pose le bandeau). Décision toujours ouverte : snippets CloudCannon `.mdx` (≈ 0,5 j) ou modèles collés | 0,25 j | — | | 2026-09-24 |
