...
| 16 sept. | **Trois branches** après une journée de collisions sur la branche d'édition (3 push rejetés, 2 builds rouges causés par des sauvegardes CMS : champs vidés enregistrés `null`, clés supprimées par les gabarits) : `spike/cloudcannon` renommée **`staging`** (édition CloudCannon, sauvegardes directes), nouvelle **`dev`** (intégration des devs, `feat/*` → PR → `dev` → PR → `staging`), `main` = production par Publish seulement. Correctifs de robustesse posés le même jour : `null → ""` dans tous les schémas, `empty_type: string` sur 222 entrées, gabarits complets + `remove_extra_inputs: false`, CI sur `staging` et `dev`. | `operations.md` §1, §4–§6 + `DEPLOYMENT.md` |
...
- **État du code** : `git log --oneline` sur la branche `staging` (édition)
  ou `dev` (intégration) ;
